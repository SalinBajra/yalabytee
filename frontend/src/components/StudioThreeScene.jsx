...
export default function StudioThreeScene() {
  ...
  useEffect(() => {
    ...
    const initScene = async () => {
      ...
      const ringMaterial = new THREE.MeshBasicMaterial({ color: 0x45d7ef, transparent: true, opacity: 0.42, blending: THREE.AdditiveBlending, side: THREE.DoubleSide });
      const violetRingMaterial = new THREE.MeshBasicMaterial({ color: 0x8d65ff, transparent: true, opacity: 0.28, blending: THREE.AdditiveBlending, side: THREE.DoubleSide });
      const rings = [
      ...
      const panelMaterial = new THREE.MeshBasicMaterial({ color: 0xd8fbff, transparent: true, opacity: 0.18, side: THREE.DoubleSide, blending: THREE.AdditiveBlending });
      const panels = [
      ...
      const particles = new THREE.Points(
        particleGeometry,
        new THREE.PointsMaterial({ color: 0x8ff4ff, size: 0.026, transparent: true, opacity: 0.68, blending: THREE.AdditiveBlending, depthWrite: false })
      );
      group.add(particles);
